...
def main():
    parser = argparse.ArgumentParser(description="Test a GGUF model using llama-cpp-python with same format as test_finetuneV2.py")
    parser.add_argument("-o", "--model_path", type=str, required=True, help="Path to the GGUF model file.")
    ...
    parser.add_argument("-c", "--n_ctx", type=int, default=4096, help="Context size for the model.")
    parser.add_argument("-g", "--n_gpu_layers", type=int, default=-1, help="Number of layers to offload to GPU (-1 = all).")
    ...
    instruction = """你是大脑中的记忆单元调度中枢，负责处理哪些记忆会被唤醒。你需要根据聊天记录、新的消息，来筛选出哪些关于能力与偏好的记忆是对回答新的消息有帮助、或对角色应对此时此刻下一秒的情景是有用的。
    ...
    4.激活的记忆总数不要超过10个
    7.请先在<think></think>标签中进行思考分析，然后再输出激活的记忆序号。
    6.思考<think>中不要输出超过2000token。
    5.若没有任何需要被激活的记忆，输出序号中输出[]空的即可例子:[2,3,19,6,8,12,37......]
    """
    ...
    for idx in range(args.test_example_idx, end_idx):
        ...
        # 4. Format the prompt using chat format (same structure as test_finetuneV2.py)
        # For GGUF models, we need to manually construct the chat format
        # Using a generic chat template structure
        prompt = f"""<|im_start|>user
{chat_item}
 memories:{memory_item}<|im_end|>
<|im_start|>system
{instruction}<|im_end|>
<|im_start|>user
next message:{message_item}<|im_end|>
<|im_start|>assistant
"""
        ...
        # 6. Extract predicted numbers and calculate metrics
