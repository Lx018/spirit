...
if len(sys.argv) == 3:
    try:
        ...
        output_filename = f"chatData/{arg_start}_{arg_end}.json"
    except ValueError:
...
for i, item in enumerate(data):
    ...
    for q in queries:
        try:
            ...
            item = {"query": q, "memory": cur_mem, "result": result, "data_idx": i}
            print("done one item")
            final.append(item)
    ...
    if (i + 1) % 10 == 0:
        print(f"Checkpoint: saving {len(final)} results to {output_filename}...")
